resource "azurerm_network_interface" "rundeck_vm_nic" {
  name                = var.rundeck_vm_nic_name
...
resource "azurerm_virtual_machine" "rundeck_vm" {
  name                  = var.rundeck_vm_name
  location              = var.cluster_vnet_location
  resource_group_name   = var.cluster_vnet_resource_group_name
  network_interface_ids = [azurerm_network_interface.rundeck_vm_nic.id]
  vm_size               = var.rundeck_vm_size
...
resource "azurerm_virtual_machine_extension" "rundeck_vm_custom_script" {
  name                 = "customScript"
  virtual_machine_id   = azurerm_virtual_machine.rundeck_vm.id
